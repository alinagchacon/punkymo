...
Las VLAN en una red conmutada permiten que usuarios de diferentes departamentos puedan conectarse a la misma red con independencia del switch físico al que estén conectados o de la ubicación en la red LAN.

<figure><img src="../../../.gitbook/assets/image (943).png" alt="" width="563"><figcaption><p>Tomado del curso de CCNA</p></figcaption></figure>
...
* cada una se considera una red lógica diferente.
* los dispositivos dentro de una VLAN funcionan como si estuvieran en su propia red aunque compartan una misma infraestructura con otras VLAN.
* Los paquetes de unidifusión, multidifusión se reenvían solo a dispositivos dentro de la VLAN. Los paquetes destinados a dispositivos fuera de la VLAN deben reenviarse a través de un dispositivo que admita enrutamiento.
...
* Proveer seguridad, segmentación, flexibilidad
* Permitir agrupar usuarios de un mismo dominio broadcast con independencia de su ubicación física en la red.
* Permitir agrupar de manera lógica puertos del switch y los usuarios conectados a los mismos se agrupan en grupos de trabajo con interés común.
* Asociar usuarios lógicamente con total independencia de la ubicación física incluso a través de redes WAN.
* Existir en un solo switch o abarcar varios utilizando los enlaces troncales para transportar el tráfico de las VLAN.
* Mejorar el rendimiento de las redes al no propagarse las difusiones de un segmento a otro. Dominios de difusión más pequeños.
* Utilizar routers puede existir comunicación entre los VLAN y también se pueden implementar ACL para mantener la seguridad necesaria.

### Diseñando VLAN

Cada VLAN en una red conmutada corresponde a una red IP. Esto hace que el diseño de la VLAN deba tener en cuenta la implementación de un esquema de direccionamiento de red jerárquico que significa que los números de red IP se aplican a los segmentos de red o a las VLAN de manera ordenada. Esto permite que la red se tome en cuenta como conjunto.

Los bloques de direcciones de red contiguas se reservan a los dispositivos en un área específica de la red.

<figure><img src="../../../.gitbook/assets/image (949).png" alt=""><figcaption><p>Tomado de CCNA</p></figcaption></figure>
